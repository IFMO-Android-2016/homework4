...
public class TimetableCache {
    ...
    private final String LOG_TAG = "TimetableCache";
    ...
    public List<TimetableEntry> get(@NonNull String fromStationId,
        ...
        Log.d(LOG_TAG, "GET version: " + version);
        TimetableDBHelper helper = TimetableDBHelper.getInstance(context, version);
        SQLiteDatabase db = helper.getReadableDatabase();
        Cursor cursor = null;
        ArrayList<TimetableEntry> list = new ArrayList<>();

        try {
            cursor = db.query(TimetableContract.TABLE,
                    version == DataSchemeVersion.V1 ? TimetableContract.V1Fields :
                            TimetableContract
                                    .V2Fields,
            ...
            if (cursor != null && cursor.moveToFirst()) {
                for (; !cursor.isAfterLast(); cursor.moveToNext()) {
                    ...
                    TimetableEntry timetableEntry = new TimetableEntry(
                            ...
                            calendar2,
                            cursor.getString(10),
                            (version == DataSchemeVersion.V2) ? cursor.getString(13) : null,
                            cursor.getString(11),
                            cursor.getString(12)
                    );
    ...
    public void put(@NonNull String fromStationId,
        ...
        try {
            if (version == DataSchemeVersion.V2) {
                ...
                for (int i = 0; i < TimetableContract.V2Fields.length; i++) {
                    args.append(TimetableContract.V2Fields[i]);
                    if (i != TimetableContract.V2Fields.length - 1) {
                        args.append(", ");
                    }
                }
                statement = db.compileStatement("INSERT INTO " + TimetableContract.TABLE + " (" +
                        args.toString() + ") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            } else {
                StringBuilder args = new StringBuilder();
                for (int i = 0; i < TimetableContract.V1Fields.length; ++i) {
                    args.append(TimetableContract.V1Fields[i]);
                    if (i != TimetableContract.V1Fields.length - 1) {
                        args.append(", ");
                    }
                }
                statement = db.compileStatement("INSERT INTO " + TimetableContract.TABLE + " (" +
            ...
            for (TimetableEntry timetableEntry : timetable) {
                ...
                if (version != DataSchemeVersion.V1 && timetableEntry.trainName != null)
                    statement.bindString(14, timetableEntry.trainName);
