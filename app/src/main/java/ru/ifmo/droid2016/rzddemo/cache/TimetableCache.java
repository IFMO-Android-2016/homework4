...
public class TimetableCache {
    ...
    public List<TimetableEntry> get(@NonNull String fromStationId,
        ...
        try {
            cursor = database.query(TimetableContract.Timetables.TABLE,
                    version == V1 ? TimetableContract.Timetables.argumentsV1 : TimetableContract.Timetables.argumentsV2,
                    TimetableContract.Timetables.FROM_STATION_ID + "=? AND " + TimetableContract.Timetables.TO_STATION_ID + "=? AND "
                            + TimetableContract.Timetables.DATE_MSK + "=?",
    ...
    public void put(@NonNull String fromStationId,
        ...
        Log.d(LOG_TAG, "put " + timetable.size());
        TimetableDBHelper timetableDBHelper = TimetableDBHelper.getInstance(context, version);
        ...
        try {
            if (version == DataSchemeVersion.V2) {
                ...
                for (int i = 0; i < TimetableContract.Timetables.argumentsV2.length; ++i) {
                    args.append(TimetableContract.Timetables.argumentsV2[i]);
                    if (i != TimetableContract.Timetables.argumentsV2.length - 1) {
                        args.append(", ");
                    }
                }
                statement = database.compileStatement("INSERT INTO " + TimetableContract.Timetables.TABLE + " (" +
                        args.toString() + ") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            } else {
                StringBuilder args = new StringBuilder();
                for (int i = 0; i < TimetableContract.Timetables.argumentsV1.length; ++i) {
                    args.append(TimetableContract.Timetables.argumentsV1[i]);
                    if (i != TimetableContract.Timetables.argumentsV1.length - 1) {
                        args.append(", ");
                    }
                }
                statement = database.compileStatement("INSERT INTO " + TimetableContract.Timetables.TABLE + " (" +
