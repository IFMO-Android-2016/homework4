...
public class TimetableCache {
    ...
    public void put(@NonNull String fromStationId,
        ...
        db.beginTransaction();
        try {
            SQLiteStatement in = db.compileStatement(toInsert);
            for (TimetableEntry entry: timetable) {
                in.bindLong(1, dateMsk.get(Calendar.YEAR) * 1000 + dateMsk.get(Calendar.DAY_OF_YEAR));
                in.bindString(2, entry.departureStationId);
                in.bindString(3, entry.departureStationName);
                in.bindLong(4, entry.departureTime.getTimeInMillis());
                in.bindString(5, entry.arrivalStationId);
                in.bindString(6, entry.arrivalStationName);
                in.bindLong(7, entry.arrivalTime.getTimeInMillis());
                in.bindString(8, entry.trainRouteId);
                in.bindString(9, entry.routeStartStationName);
                in.bindString(10, entry.routeEndStationName);
                if (version == DataSchemeVersion.V2) {
                    if (entry.trainName == null) {
                        in.bindNull(11);
                    } else {
                        in.bindString(11, entry.trainName);
                    }
                }
                in.executeInsert();
            }
